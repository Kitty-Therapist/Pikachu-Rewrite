...
from utils import Utils, BotLogs, Configuration
...
async def on_command_error(ctx: commands.Context, error):
    BOT_LOG_CHANNEL = bot.get_channel(632297974503964686)
...
async def on_error(event, *args, **kwargs):
    ...
    BOT_LOG_CHANNEL = bot.get_channel(632297974503964686)
    BOT_LOG_CHANNEL.send(f"error in {event}\n{args}\n{kwargs}")
    embed = discord.Embed(colour=discord.Colour(0xff0000),
                          timestamp=datetime.datetime.utcfromtimestamp(time.time()))
...
@bot.event
async def on_ready():
    await bot.change_presence(activity=discord.Activity(name='Pika Pika!', type=discord.ActivityType.watching))
